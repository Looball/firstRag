...
def get_user_llm_provider_credential(
    user_id: int,
    provider: str,
) -> Row | None:
    """查询当前用户指定厂商的加密 API 凭据。"""
    return fetch_one(
        """
        SELECT
            user_id,
...
def get_user_llm_provider_credentials(user_id: int) -> list[Row]:
    """查询当前用户已保存的厂商凭据元数据。"""
    return fetch_all(
        """
        SELECT
...
def upsert_user_llm_provider_credential(
    user_id: int,
    provider: str,
    credential: dict[str, Any],
) -> Row | None:
    """新增或更新当前用户指定厂商的加密 API 凭据。"""
    return fetch_one(
        """
