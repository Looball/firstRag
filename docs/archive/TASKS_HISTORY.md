# FirstRAG 历史任务详情与验收记录

本文件是 `docs/TASKS.md` 的历史详情归档，保留任务目标、范围、验收标准、完成记录和验证命令。当前任务状态以 [`../TASKS.md`](../TASKS.md) 为准；本文件不单独维护新的任务状态。
...
- 相关提交：`12852ff`。
- 完成记录：
  - OCR report schema 升级为 v2，增加经过字段白名单限制的 runner OS、架构、run/attempt、commit 和 ref context；历史 JSON 再执行独立 allowlist 投影，只保留整体/逐 case 质量、耗时、策略和 gate 状态，不保存期望正文、OCR 正文或任意扩展字段。
  - 新增 `pdf_ocr_trend.py`，支持历史校验、CI rerun 去重、最多 50 条有界清理、损坏文件 warning、同 runner OS/架构/Tesseract 完整版本过滤，以及以本次运行锚定的最近 5 次中位数趋势；耗时 `1.25x/1.50x` 和质量下降 `0.01/0.02` 分别显示 `WATCH/REGRESSED`，但不取代当前硬门禁退出码。
  - GitHub Actions backend job 用 cache 恢复同 runner 历史，把当前 JSON、Markdown、trend 作为 artifact 保留 30 天，并把趋势写入 Step Summary；cache 与 artifact Action 分别固定到官方 `v4.2.4`、`v4.6.2` 完整 commit SHA，Action pin policy 通过 `9` 个引用。
  - 本机 Tesseract 5.5.2 连续两轮真实评测均为 5/5 case、宏平均相似度 `1.0000`，总耗时 `21.708s -> 21.651s`；第二轮趋势为 `STABLE`、可比运行 `2`、耗时比 `1.00x`。最终 Compose backend 评测同样 5/5 通过、宏平均相似度 `1.0000`、总耗时 `16.190s`，并成功生成 baseline 历史和趋势。
  - 新增 7 个趋势专项测试，覆盖跨环境过滤、未来时间戳锚定、质量/耗时阈值、硬门禁失败、损坏/未来 schema 降级、rerun 去重、有界清理和敏感扩展字段拒绝；OCR 专项 15 项、后端全量 364 项测试均通过。
  - 完整 Compose 镜像重建成功，PostgreSQL、Redis、Chroma healthy，migration `applied=0 skipped=9`，backend、worker、frontend 正常；production preflight、Chroma runtime health、migration dry-run、Compose config、CI YAML、compileall 和 `git diff --check` 均通过。
- 建议验证命令：
...
cd backend && conda run -n firstrag env PYTHONPATH=. python -m unittest tests.services.test_pdf_ocr_benchmark tests.services.test_pdf_ocr_trend -v
...
- 相关提交：`378f206`。
- 完成记录：
  - 新增 `ConversationSidebar.tsx`，将新建会话入口、会话计数、空状态、活动项、消息摘要、重命名和删除 UI 从主页面抽离；会话请求、选择和异步 lifecycle state 继续由 `page.tsx` 管理，没有新增 effect 或请求 waterfall。
...
- 相关提交：`32c0b99`。
- 完成记录：
  - 新增 `KnowledgeBaseSidebarControls.tsx`，将知识库选择、管理入口、上传按钮、Retry-After 文案、文件计数和隐藏 file input 从主页面抽离；知识库、文件请求和异步 state 继续由页面与现有 hook 管理。
  - 页面与文件管理弹窗继续共享同一个 `fileInputRef`，没有复制上传入口，也没有新增 effect、数据请求或请求 waterfall。
  - `page.tsx` 从 2862 行降至 2804 行，减少 58 行；新增 3 项组件静态渲染测试，覆盖空知识库、选择项、文件计数、上传中和限流状态。
  - 前端全量 Vitest 18 个文件、104 项通过，lint 0 error 并保留 2 个既有 `<img>` warning；宿主机与 Docker 中的 Next.js 16.2.12 production build、Playwright E2E 3/3 均通过。
...
## T-097 拆分侧栏用户身份与模式控制组件

- 来源计划：`PLAN-20260728-07`
- 优先级：`P1`
- 状态：`Done`
- 背景：T-096 完成后 `frontend/src/app/page.tsx` 仍有 2804 行，侧栏顶部的用户身份、设置入口、退出操作和普通/高级模式切换继续以内联 JSX 形式存在。
- 目标：在不改变认证、退出和模式偏好行为的前提下，将侧栏账户与模式控制区迁移到独立、可测试的组件边界。
- 技术边界：
  - 当前用户名、认证清理、退出跳转、模式 state 和 `localStorage` 偏好持久化继续由 `page.tsx` 管理。
  - 子组件只接收用户名、模式状态和 callbacks，不增加 state、effect、浏览器存储访问或数据请求。
  - 保持设置页链接、用户名回退文案、退出入口和 `aria-pressed` 语义不变。
- 范围：
  - 新增 `SidebarAccountModeControls.tsx`，承接用户身份、设置/退出入口和普通/高级模式按钮。
  - `page.tsx` 改为复用独立组件，并保留认证与模式偏好流程。
  - 增加组件静态渲染测试，覆盖用户名、回退文案、设置入口和两种模式状态。
  - 更新前端结构文档，明确页面与侧栏账户模式组件的职责边界。
- 验收标准：
  - `page.tsx` 至少减少 45 行，认证和模式偏好 state 与副作用仍保留在页面层。
  - 新增组件测试、前端全量 Vitest、lint、production build 和 Playwright E2E 通过。
  - Docker Compose、服务日志和 production preflight 通过。
- 相关提交：`2ed46af`。
- 完成记录：
  - 新增 `SidebarAccountModeControls.tsx`，将用户身份、设置入口、退出操作和普通/高级模式按钮从主页面抽离；当前用户名、认证清理、退出跳转、模式 state 和浏览器偏好持久化继续由 `page.tsx` 管理。
  - 子组件保持纯展示职责，没有新增 state、effect、浏览器存储访问或数据请求；设置链接、空用户名回退文案和 `aria-pressed` 语义保持不变。
...
- 相关提交：`0f9ee9b`。
- 完成记录：
  - 新增 `ChatComposer.tsx`，将 textarea、待发送图片预览、图片选择入口、附件约束说明和发送按钮从主页面抽离；输入 state、图片校验与 URL 清理、粘贴解析、上传、会话创建、限流倒计时和发送流程继续由 `page.tsx` 管理。
  - 将图片 MIME、数量、大小约束和 `PendingChatImage` 类型集中到组件模块，页面与展示层复用同一约束；子组件没有新增 state、effect、数据请求或异步业务流程。
  - 保持 Enter/Shift+Enter、图片按钮和发送按钮禁用条件、创建/上传/思考/限流状态文案优先级不变；`page.tsx` 从 2728 行降至 2587 行，减少 141 行。
...
- 相关提交：`d41d5aa`。
- 完成记录：
  - 新增 `MessageSourceList.tsx`，将引用摘要、文件位置、OCR 状态、检索 diagnostics、原文入口和 source feedback 从消息循环抽离；原文弹窗 state、feedback API、消息状态回写和临时提示计时继续由 `page.tsx` 管理。
  - 将引用位置与 OCR confidence 格式化调用移入组件模块；子组件仅接收 sources、显示计数、feedback 状态和 callbacks，没有新增 state、effect 或数据请求。
  - 保持普通/高级模式差异、检索分数四位小数、OCR 校对 metadata、原文入口条件及错误优先于成功提示的行为不变；`page.tsx` 从 2587 行降至 2410 行，减少 177 行。
...
- 相关提交：`90b61cc`。
- 完成记录：
  - 新增 `AssistantMessageActions.tsx`，将回答反馈、负向原因表单、Eval 草稿、diagnostics 和复制操作从消息循环抽离；feedback API、消息状态回写、diagnostics 请求与缓存、文件导出、复制实现和提示计时继续由 `page.tsx` 管理。
  - 复用 `MessageDiagnosticsPanel` 并将其内部 import 调整为 Vitest 可直接解析的相对路径；新组件仅接收显示状态和 callbacks，没有新增 state、effect 或数据请求。
...
- 技术边界：
  - `sessions` 继续由 `page.tsx` 持有，hook 仅接收稳定的 `setSessions` 并回写目标 session/message。
  - feedback API、source index 匹配、错误/成功提示计时、Eval JSON 序列化和 Blob 下载迁入 hook。
  - 保持未持久化消息提示、negative feedback 默认 reason、note trim、错误文案、2 秒成功提示和高级模式门禁不变。
- 范围：
  - 新增 `use-message-quality-actions.ts`，管理 message/source feedback 与 Eval 草稿 state 和 actions。
  - 抽取纯 helper，覆盖消息反馈回写、显式/位置 source index 匹配和 Eval 文件序列化。
  - `page.tsx` 改为消费 hook 返回值，并继续负责消息组件装配。
...
- 验收标准：
  ...
  - Docker Compose、服务日志和 production preflight 通过。
- 完成记录：
  - 新增 `use-knowledge-base-lifecycle.ts`，集中管理管理弹窗、回收站加载、创建、重命名、移入回收站、恢复及 12 组局部 state。
  - `knowledgeBases`、当前知识库 ID、`sessions` 和当前会话 ID 仍由页面持有；retrieval settings、文件管理和初次认证加载保持独立边界。
  - 删除继续禁止默认知识库并保留会话数量确认文案；删除/恢复后的活动列表与回收站请求通过 `Promise.all` 并行刷新。
  - 刷新后保持“恢复目标优先、当前选择其次、默认知识库兜底”的选择顺序，并保留仍可见的当前会话。
  - 创建和重命名继续 trim 名称、保留既有 loading、错误/成功文案和输入清理行为。
  - `page.tsx` 从 1392 行降至 1185 行，减少 207 行。
  - 新增 7 项 helper 测试；前端全量 Vitest 共 31 个测试文件、160 项通过。
  - lint 0 error 并保留 2 个既有 `<img>` warning；宿主机与 Docker production build、Playwright E2E 3/3 均通过。
  - Docker runtime audit 输出 `found 0 vulnerabilities`；Compose 核心服务状态正常，migration 输出 `applied=0 skipped=9`，production preflight 全部通过。
- 相关提交：`c18e3c3`
- 建议验证命令：
...
- 验收标准：
  ...
  - Docker Compose、服务日志、production preflight 和隔离 full-stack E2E 通过。
- 完成记录：
  - 新增 `use-knowledge-file-mutations.ts`，集中管理上传、关联、解除关联、永久删除及其 loading/error/Retry-After 状态。
  - `useKnowledgeFileLibrary` 仅保留文件集合、知识库关联缓存、列表加载与刷新、文件数回写和派生集合；`useKnowledgeFiles` 组合 library、mutations 与 indexing，并保留跨域提示和 index queue 清理。
  - 上传成功/error helper 与测试迁移到 mutation 模块，上传大小/类型提示、操作去重、input 清空和既有 API 调用顺序保持不变。
  - `use-knowledge-file-library.ts` 从 445 行降至 200 行，减少 245 行；新 mutation hook 为 262 行，顶层 `use-knowledge-files.ts` 为 189 行。
  - 前端全量 Vitest 共 36 个测试文件、180 项通过；lint 0 error 并保留 2 个既有 `<img>` warning。
...
- 完成记录：
  - `README.md` 已将 FirstRAG 定位为可运行的全栈 RAG 工程教程与参考实现，Roadmap 切换到 T-122 至 T-129 教程化批次，并如实保留在线 demo 的外部阻塞状态。
  - `docs/README.md`、`AGENTS.md` 与 `docs/AGENT_GUIDE.md` 已统一功能冻结、`main` 单长期主线、短期任务分支和教程真实性边界。
  - 本地与远端 annotated tag `product-v1.0.0` 的 peeled commit 均为 `24a236749369d0b89f7c4a8bd5afb998fdde68f5`，对应计划合并后、教程内容改造前的完整产品基线。
  - 计划 PR #42 的 `Backend`、`Frontend`、`Full-stack E2E`、`Container OS Security` 全部通过，review threads 为空，并已 squash merge；本任务未修改业务代码、API、数据库或部署配置。
- 相关提交：`dd3f2c4`
- 建议验证命令：
...
- 完成记录：
  - 根 `README.md` 已增加学习收获、四条学习路线、10 分钟导览和源码地图入口，并明确无密钥交互实验仍由 T-124 交付。
  - `docs/tutorials/README.md` 已提供快速入门、后端/RAG、前端和工程化四条路线；每条路线均说明适合读者、先修条件、预计产出、阅读顺序和可跳过内容。
  - `docs/tutorials/CODE_MAP.md` 已按入库、检索、前端、安全、OCR、测试和部署组织纵向源码入口，覆盖 route、repository、service、worker、proxy、component、hook 与验证脚本。
  - `docs/README.md` 已区分 tutorial、reference、runbook、evaluation、协作任务和 historical material；未完成专题只标记任务状态，没有创建空白章节。
  - 对本次变更中的 Markdown 本地路径和跨页锚点执行了自动检查，结果全部通过；`git diff --check` 无错误。
- 相关提交：`1aab220`
- 建议验证命令：
...
- 验收标准：
  ...
  - 未获得授权前不删除原有版权和限制声明。
- 完成记录：
  - 仓库所有者在比较当前限制、MIT 与 Apache-2.0 后，于 2026-08-03 明确确认采用 Apache License 2.0。
  - 根目录 `LICENSE` 已替换为 Apache 官方完整 2.0 文本；去除官网响应额外的首个空行后，本地与官方文件均为 11,357 bytes，SHA-256 为 `c71d239df91726fc519c6eb72d318ec65820627232b2f796219e87dcf35d0ab4`。
  - 新增 `NOTICE` 保存 `Copyright 2026 FirstRAG contributors` 归属；README 与教程入口明确代码、文档和仓库自编 fixture 的授权，以及修改、分发、商业使用、托管服务、商标和无担保边界。
  - 第三方依赖、外部 provider 服务条款、用户上传内容、API Key、数据库和日志被明确排除在仓库 Apache-2.0 再授权之外。
  - `frontend/package.json` 与 lockfile 根 package 均声明 SPDX identifier `Apache-2.0`，且没有冲突的 License badge 或其他 package metadata。
  - 教程文档检查 8 篇、针对性后端测试 9 项、GitHub Actions pin 13 项、License metadata 一致性与 `git diff --check` 通过。
  - `docker compose up -d --build` 通过，frontend production build 成功，migration 为 `applied=0 skipped=9`；当前启动日志、backend health 和 frontend 登录页 smoke 无错误。
- 相关提交：`1d66209`
- 建议验证命令：
