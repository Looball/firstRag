...
- Redis：提供基础设施健康检查、RAG 热点共享缓存和后端分布式限流，包括知识库画像、retrieval settings、query embedding 和登录/业务 API sliding-window 计数；后续承接 worker 运行态，不作为 vector index job 的持久队列。
