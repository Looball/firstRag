...
用户提问
  ...
  -> Milvus dense+sparse hybrid/RRF -> child rerank -> Milvus parent context
...
回答引用原文预览
  ...
  -> 当前用户 JWT 权限校验
  -> Milvus 当前 user/file/index_version entities
  -> 返回目标 chunk、相邻上下文及 PDF 页码或 DOCX 段落范围
  -> 校对工作台按权限将 PDF 目标页即时渲染为 PNG；新窗口原文件使用 #page=N 跳页

文件级 OCR 质量巡检
  -> 当前用户选择已索引 PDF
  -> Milvus 当前 index_version 的 OCR 代表 entities + PostgreSQL corrections/history 摘要
...
| Sparse encoder | `backend/sparse_encoder` | Compose 内网单实例加载固定 revision BGE-M3，提供 document/query learned sparse contract；worker 用于写入，backend 用于 hybrid query。 |

## 存储组件

- PostgreSQL：用户、知识库、文件 metadata、会话、消息、聊天附件 metadata、向量化任务队列、OCR correction/history 和模型设置；不保存检索 parent/child 正文。
- PostgreSQL OCR corrections：按用户、文件和页码保存人工修订、原始 OCR 文本与 revision；知识文件永久删除时级联清理。
- PostgreSQL OCR history：按用户、文件、页码和 index version 保存有上限的 Tesseract 最佳原始识别记录、质量指标、文本 SHA、所选策略、候选摘要和来源 job；与 Milvus entities 生命周期解耦，文件删除时级联清理。
...
- Milvus：唯一受支持的检索文本与向量存储。v3 entity 同时保存 dense/sparse vectors、child text、parent text、stable IDs 与位置 metadata；Compose 启动 authenticated Standalone、etcd、MinIO 和一次性 health probe，Strong consistency、写后文本对账与 dense/sparse self-hit 保证跨进程可见。
