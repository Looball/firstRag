...
具备 `file_id` 和 `chunk_index` 的引用卡片会显示“查看原文”。点击后按需加载 `SourcePreviewDialog`，并通过 React Query 按 `file_id + chunk_index + radius` 缓存请求；弹窗高亮目标 chunk、展示相邻上下文、标题层级和 PDF 页码或 DOCX 段落范围。打开 PDF 原始文件时 blob URL 会附加 `#page=N` 跳到目标页；浏览器无法可靠控制 DOCX 内部光标，因此 DOCX 只在内置弹窗中高亮并展示段落范围。历史 source 缺少定位字段时保留现有摘要，不展示不可用入口。
