...
`npm run test:e2e:full-stack` 使用独立 Playwright 配置连接已启动的隔离 Compose 环境。标准入口为仓库根目录的 `scripts/run_full_stack_e2e.sh`：默认创建临时 PostgreSQL、Milvus 及 uploads volumes，也可显式以 `VECTOR_STORE_PROVIDER=chroma` 覆盖回滚 adapter；本地确定性 OpenAI-compatible stub 提供 chat/embedding，先通过真实注册 API 创建测试账号，再由 Chromium 完成登录、TXT 上传、worker 向量化、SSE 回答和引用展示。该门禁不读取真实账号、`.env` provider Key 或公网模型服务，结束后只删除自己的 `firstrag-t089-*` Compose project。
...
- `page.tsx`：知识库、文件、会话和聊天主工作台；知识库管理支持重命名、移入回收站和恢复，文件管理支持解除单个知识库关联或经二次确认永久删除全局文件；知识文件上传支持 PDF、DOCX、Markdown、TXT、PNG、JPEG 和 WebP，图片入库向量化失败时展示 vision 模型配置恢复动作。认证检查、用户名、初始知识库/会话恢复和可见会话选择回退由 `lib/chat-workspace/use-workspace-bootstrap.ts` 管理，核心 `knowledgeBases`/`sessions` 与当前 ID 仍由页面持有。知识文件列表、关联缓存、加载和派生集合由 `lib/chat-workspace/use-knowledge-file-library.ts` 管理，上传、关联、解除关联、永久删除和上传 Retry-After 状态由 `lib/chat-workspace/use-knowledge-file-mutations.ts` 管理，vector index health、单文件/整库索引、删除向量和 Retry-After 状态由 `lib/chat-workspace/use-knowledge-file-indexing.ts` 管理，本地任务队列、任务等待、刷新、清理和两段后台轮询由 `lib/chat-workspace/use-vector-index-queue.ts` 管理，`lib/chat-workspace/use-knowledge-files.ts` 仅组合 library、mutations 与 indexing 并协调跨域刷新和提示。单条消息的角色外观、流式占位、检索空态和子组件组合由 `components/chat-workspace/ConversationMessageItem.tsx` 独立渲染；其内部复用 `AssistantMessageActions.tsx`、`MessageSourceList.tsx` 和 `MessageContent.tsx`。message feedback 面板/草稿/提交与 Eval 草稿导出由 `lib/chat-workspace/use-message-quality-actions.ts` 管理，source feedback 提交、loading/error/message、提示计时和目标引用回写由 `lib/chat-workspace/use-source-feedback-actions.ts` 管理，conversation diagnostics 的缓存、silent preload、首次展开加载和错误状态由 `lib/chat-workspace/use-conversation-diagnostics.ts` 管理，回答复制的 Clipboard API、textarea fallback 和 1.5 秒提示由 `lib/chat-workspace/use-message-clipboard.ts` 管理，会话创建、选择、重命名和删除由 `lib/chat-workspace/use-conversation-actions.ts` 管理，active session 消息懒加载、取消保护和 session error 回写由 `lib/chat-workspace/use-conversation-message-loader.ts` 管理，聊天自动建会话、图片上传、用户消息事务和提交互斥由 `lib/chat-workspace/use-chat-submission.ts` 管理，chat 请求、SSE assistant content/message ID/sources/retrieval/fallback 回写、请求错误/限流、loading 清理和高级模式 diagnostics preload 由 `lib/chat-workspace/use-chat-response-stream.ts` 管理，知识库管理弹窗开关、创建和重命名由 `lib/chat-workspace/use-knowledge-base-lifecycle.ts` 管理，回收站加载、软删除、恢复、集合刷新和选择回退由 `lib/chat-workspace/use-knowledge-base-trash-actions.ts` 管理，知识库 retrieval settings 的按库缓存、加载、更新、保存和提示状态由 `lib/chat-workspace/use-knowledge-base-retrieval-settings.ts` 管理。聊天输入、待发送图片和发送状态由 `components/chat-workspace/ChatComposer.tsx` 独立渲染，待发送图片的数量/类型/大小校验、剪贴板筛选、预览和 Object URL 生命周期由 `lib/chat-workspace/use-pending-chat-images.ts` 管理。主工作区标题与消息/文件统计由 `components/chat-workspace/ChatWorkspaceHeader.tsx` 独立渲染，用户身份、设置/退出入口和模式切换由 `components/chat-workspace/SidebarAccountModeControls.tsx` 独立渲染，知识库管理弹窗由 `components/chat-workspace/KnowledgeBaseManagerDialog.tsx` 独立渲染，知识库选择与文件入口由 `components/chat-workspace/KnowledgeBaseSidebarControls.tsx` 独立渲染，会话索引由 `components/chat-workspace/ConversationSidebar.tsx` 独立渲染，高级模式质量指标由 `components/chat-workspace/QualityDashboardPanel.tsx` 独立渲染，其展开、缓存、首次加载和刷新流程由 `lib/chat-workspace/use-quality-dashboard.ts` 管理；页面保留模式偏好、原文弹窗以及各业务 hooks 的装配。
- `lib/chat-workspace/vector-index-utils.ts`：知识文件与 vector job 响应解析、文件向量状态、失败恢复动作、worker/queue health 解析和任务状态展示 helper；消费方直接导入该模块，通用 `utils.ts` 不再承载 vector indexing 领域逻辑。
