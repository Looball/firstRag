...
`npm run test:e2e:full-stack` 使用独立 Playwright 配置连接已启动的隔离 Compose 环境。标准入口为仓库根目录的 `scripts/run_full_stack_e2e.sh`：创建临时 PostgreSQL、Milvus 及 uploads volumes；本地确定性 OpenAI-compatible stub 提供 chat/embedding，先通过真实注册 API 创建测试账号，再由 Chromium 完成登录、TXT 上传、worker 向量化、SSE 回答和引用展示。该门禁不读取真实账号、`.env` provider Key 或公网模型服务，结束后只删除自己的 `firstrag-t089-*` Compose project。
