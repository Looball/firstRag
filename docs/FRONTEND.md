...
- `page.tsx`：知识库、文件、会话和聊天主工作台；知识库管理支持重命名、移入回收站和恢复，文件管理支持解除单个知识库关联或经二次确认永久删除全局文件；知识文件上传支持 PDF、DOCX、Markdown、TXT、PNG、JPEG 和 WebP，图片入库向量化失败时展示 vision 模型配置恢复动作。回答反馈、Eval 草稿、diagnostics 和复制操作由 `components/chat-workspace/AssistantMessageActions.tsx` 独立渲染，引用来源、原文入口和 source feedback 由 `components/chat-workspace/MessageSourceList.tsx` 独立渲染，聊天输入、待发送图片和发送状态由 `components/chat-workspace/ChatComposer.tsx` 独立渲染，主工作区标题与消息/文件统计由 `components/chat-workspace/ChatWorkspaceHeader.tsx` 独立渲染，用户身份、设置/退出入口和模式切换由 `components/chat-workspace/SidebarAccountModeControls.tsx` 独立渲染，知识库管理弹窗由 `components/chat-workspace/KnowledgeBaseManagerDialog.tsx` 独立渲染，知识库选择与文件入口由 `components/chat-workspace/KnowledgeBaseSidebarControls.tsx` 独立渲染，会话索引由 `components/chat-workspace/ConversationSidebar.tsx` 独立渲染，高级模式质量指标由 `components/chat-workspace/QualityDashboardPanel.tsx` 独立渲染，消息 Markdown 与认证图片附件由 `components/chat-workspace/MessageContent.tsx` 独立渲染；页面保留认证、模式偏好、图片校验与 Object URL 生命周期、上传发送流程、原文弹窗 state、feedback 请求与消息回写、diagnostics 请求与缓存、Eval 文件导出、复制实现和提示计时，以及相关请求、生命周期状态、sources、诊断和流式状态编排。
