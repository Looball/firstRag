...
| `GET` | `/chat/knowledge-files/{knowledge_file_id}/pages/{page_number}/preview` | 将当前用户 PDF 目标页即时渲染为受限尺寸 PNG。 |
| `GET` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages` | 返回当前索引版本的 OCR 页面质量清单、待处理统计和人工修订状态。 |
| `GET` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/history` | 返回指定 OCR 页的识别历史、质量趋势和相邻文本变化。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/reindex` | 把多个 OCR 页合并为一个异步重新识别批次并重建文件索引。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/reindex` | 异步重新识别指定 OCR 页面并重建文件索引。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/reindex-jobs/{job_id}/retry` | 使用失败 OCR job 保存的原页码批次重新排队。 |
| `GET` / `PATCH` / `DELETE` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/correction` | 读取、保存或撤销指定 OCR 页的人工修订；修改操作异步重建索引。 |
...
页级人工校对接口读取完整页面正文，并按 `user_id + knowledge_file_id + page_number` 持久化修订。`PATCH` 请求体为 `{ "corrected_text": "..." }`，去除首尾空白后必须为 1-50000 个字符，且不能与当前文本相同；`DELETE` 撤销修订。两种修改都会递增文件 `index_version` 并通过 vector job 重建完整文件索引。worker 仍运行 Tesseract 以保留 `ocr_confidence`，再在切分前应用人工文本；新 chunks/source 通过 `ocr_correction_applied`、`ocr_correction_revision`、`ocr_correction_updated_at` 标识修订。历史回答引用继续绑定旧版本。

引用原文定位使用 `knowledge_file_id + chunk_index + index_version`；新生成的 source 会持久化其索引版本，旧 source 未携带版本时回退到最新可用 chunk 版本。chunk 上下文接口只返回完整正文和白名单位置 metadata（`h1`-`h6`、PDF 的 `page_index/page_number/page_count` 与 OCR 解析信息、DOCX 的 `paragraph_start/paragraph_end`），不返回 `storage_path`。PDF 页码来自逐页解析结果，使用 1-based `page_number`；DOCX 段落号来自 `word/document.xml` 的原始文档顺序，同样从 1 开始。其他格式没有稳定位置时，前端只展示精确 chunk，不生成推测页码。原始文件接口不信任上传时的 Content-Type，而是按受支持扩展名设置安全 MIME，并返回 `nosniff` 和 sandbox 响应头；PDF 页预览接口在同样的用户权限和 uploads 路径边界内使用 PyMuPDF 渲染 RGB PNG，限制最长边为 1800px，以 `private, max-age=60` 返回且不落盘。跨用户、路径越界、文件缺失或已重新索引后目标 chunk 不存在时返回 `404`。
...
| `GET` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages` | 读取 PDF 当前索引版本的 OCR 质量巡检清单。 |
| `GET` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/history` | 读取 PDF 页级 OCR 识别历史。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/reindex` | 提交 PDF 多页 OCR 重识别批次。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/reindex` | 提交 PDF 单页 OCR 重识别任务。 |
| `POST` | `/chat/knowledge-files/{knowledge_file_id}/ocr/reindex-jobs/{job_id}/retry` | 按原 job options 重试失败 OCR 批次。 |
| `GET` / `PATCH` / `DELETE` | `/chat/knowledge-files/{knowledge_file_id}/ocr/pages/{page_number}/correction` | 管理 PDF OCR 页级人工修订。 |
