...
- FirstRAG 已进入“功能冻结、教程优先”阶段，默认不再增加非必要产品功能，也不为了缩短文件行数继续拆分职责已经清晰的模块。
- `main` 是唯一长期主线；每项修改从 `main` 创建短期 `codex/...` 分支，通过 PR、required checks 和 squash merge 回到 `main`，不建立长期 `tutorial` 分支。
- Bug、安全漏洞、依赖兼容和教程可复现性所需的最小修复仍属于正常维护范围。
- 教程和示例必须引用真实实现、真实 API 与可重复验证路径；不得用平行 demo 替代生产链路或把计划能力写成已实现功能。
- 教程化前产品快照由 `product-v1.0.0` tag 固定，历史任务与验收记录继续保留在 `docs/TASKS.md`。
...
- 所有项目修改都从最新 `main` 创建短期分支，分支名前缀优先使用 `codex/`，若本地命名空间冲突可使用 `codex-*`。
- 不直接提交受保护的 `main`，也不建立与 `main` 长期并行的教程开发分支。
...
- 默认先运行 `docker compose up -d --build`，再检查 `docker compose ps` 和 Redis、PostgreSQL、Milvus（含 etcd/MinIO 与 authenticated probe）、BGE-M3 sparse encoder、migration、backend、worker、frontend 日志。
- 涉及部署、RAG、上传或向量化时，运行 `scripts/production_preflight.py --check-runtime-health`，确认 Milvus、固定 BGE-M3 sparse encoder、Compose 拓扑、资源和 authenticated runtime health。
