...
- 修改 PDF OCR engine、预处理、候选策略、语言包或 Tesseract runtime 时，必须运行 `conda run -n firstrag python scripts/eval_pdf_ocr.py`；一键验收默认包含该门禁，只有与 OCR 完全无关且本机没有 Tesseract 时才使用 `--skip-ocr-eval`。
