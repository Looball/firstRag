...
- 默认先运行 `docker compose up -d --build`，再检查 `docker compose ps` 和 Redis、PostgreSQL、Milvus（含 etcd/MinIO 与 authenticated probe）、migration、backend、worker、frontend 日志。
- 涉及部署、RAG、上传或向量化时，运行 `scripts/production_preflight.py --check-runtime-health`，确认 Milvus 配置、Compose 拓扑、资源和 authenticated runtime health。
