...
dependency audit policy 在 PR、`main` push、手动触发和每周一计划任务中执行。`high` / `critical` finding 永远阻断，未 triage 的 `moderate` 也会阻断；已确认不可达的 moderate 只能在 `security/npm-audit-exceptions.json` 中登记最长 31 天的限时例外。例外过期、finding severity 上升，或上游修复后遗留无效例外都会让 CI 失败。详细维护流程见 `security/README.md`。
