...
| `PDF_OCR_*` | 扫描 PDF 本地 OCR 开关、语言、DPI、单页超时、原生文本阈值和单文件最大 OCR 页数；Compose 镜像内置 Tesseract `chi_sim/eng`。 |
...
| `PDF_OCR_ENABLED` / `PDF_OCR_MAX_PAGES` | 公开 demo 建议保持本地 OCR 开启并限制最大 OCR 页数，防止超长扫描件占满 worker CPU。 |
