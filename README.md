...
FirstRAG 是一个全栈 RAG（Retrieval-Augmented Generation，检索增强生成）应用，用于构建本地知识库问答系统。项目支持用户注册登录、知识库管理、文件上传、图片知识文件解析、异步向量化、混合检索、模型设置、聊天图片附件和流式回答。
...
4. 在“文件”弹窗中触发向量化，等待任务队列完成。图片文件会先由当前用户的 vision 聊天模型解析为可检索 Markdown，再进入向量与全文检索。
...
| 后端 | FastAPI, Pydantic |
| 数据库 | PostgreSQL |
| 缓存基础设施 | Redis（健康检查、RAG 热点共享缓存和后端分布式限流；后续承接 worker 运行态） |
| 向量库 | Chroma |
