...
const pdfOcrQualityReportSchema = z
  .object({
    ...
    pages: z.array(z.object({
      ...
      ocr_strategy: z.string().default("baseline_auto"),
      ocr_preprocessing: z.string().default("color"),
      ocr_psm: z.number().int().nonnegative().default(3),
      ocr_rotation: z.number().int().nonnegative().default(0),
      ocr_candidate_count: z.number().int().nonnegative().default(0),
...
const pdfOcrHistoryReportSchema = z
  .object({
    ...
    runs: z.array(z.object({
      ...
      ocr_rotation: z.number().int().nonnegative().default(0),
      ocr_candidate_count: z.number().int().nonnegative().default(0),
      ocr_candidate_results: z.array(z.object({
        strategy: z.string(),
        preprocessing: z.string(),
        psm: z.number().int().nonnegative(),
        rotation: z.number().int().nonnegative(),
        status: z.enum(["succeeded", "failed", "skipped"]),
        confidence: z.number().nullable(),
        word_count: z.number().int().nonnegative(),
        effective_characters: z.number().int().nonnegative(),
        text_sha256: z.string().length(64).nullable(),
        selected: z.boolean(),
      })).default([]),
      created_at: z.string(),
...
export async function loadPdfOcrQualityReport(
  ...
  return {
    ...
    pages: parsed.data.pages.map((page) => ({
      ...
      ocrStrategy: page.ocr_strategy,
      ocrPreprocessing: page.ocr_preprocessing,
      ocrPsm: page.ocr_psm,
      ocrRotation: page.ocr_rotation,
      ocrCandidateCount: page.ocr_candidate_count,
...
export async function loadPdfOcrPageHistory(
  ...
  return {
    ...
    runs: parsed.data.runs.map((run) => ({
      ...
      ocrStrategy: run.ocr_strategy,
      ocrPreprocessing: run.ocr_preprocessing,
      ocrPsm: run.ocr_psm,
      ocrRotation: run.ocr_rotation,
      ocrCandidateCount: run.ocr_candidate_count,
      ocrCandidateResults: run.ocr_candidate_results.map((candidate) => ({
        ...
        effectiveCharacters: candidate.effective_characters,
        textSha256: candidate.text_sha256,
